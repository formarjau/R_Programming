...
eData_GSEXXXX <- oligo::read.celfiles(filenames=files) #Loading data.
```
...
```{r}
GSEXXXX_Eset_Raw <- oligo::rma(eData_GSEXXXX,background = FALSE,normalize = FALSE,target = "extended")
...
```{r}
GSEXXXX_Eset_Proc <- oligo::rma(eData_GSEXXXX,normalize = TRUE,target = "extended")
